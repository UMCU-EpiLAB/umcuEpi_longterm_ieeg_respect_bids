function [vol, M, mr_parms, volsz] = load_mgh_windows(fname,slices,frames,headeronly)
% [vol, M, mr_parms, volsz] = load_mgh_windows(fname,<slices>,<frames>,<headeronly>)
%
...
%    $Revision: 1.22 $
%
% changed to work in windows as well 24-9-2025 Thomas Martens and Susanne
% Jelsma
...
if(nargin < 1 | nargin > 4)
  msg = 'USAGE: [vol M] = load_mgh_windows(fname,<slices>,<frames>,<headeronly>)';
  fprintf('%s',msg);
  return;
end
% 
...
%   %ind = findstr(fname, '.');
%   %new_fname = sprintf('/tmp/tmp.load_mgh_windows.%d.mgh', gzipped);
%   new_fname = sprintf('%s.load_mgh_windows.%d.mgh', tempname,gzipped);
% 
...
% unzip if it is compressed 
if (strcmpi(fname((end-3):end), '.MGZ') || ...
    strcmpi(fname((end-2):end), '.GZ'))
  ...
  % Use MATLAB's gunzip instead of zcat/gunzip
  tmpdir = tempname; 
  mkdir(tmpdir); % create a temp folder
  gunzip(fname, tmpdir); % decompress into it

  % Get the unzipped file name (same as original, without .gz/.mgz)
  [~, name, ~] = fileparts(fname);
  unzipped_file = fullfile(tmpdir, name);

  % Move/rename to the expected new_fname
  movefile(unzipped_file, new_fname);
...
if(slices(1) > 0)
  ...
  if(~isempty(ind))
    fprintf('ERROR: load_mgh_windows: some slices exceed nslices\n');
    return;
  end
end
...
if(frames(1) > 0)
  ...
  if(~isempty(ind))
    fprintf('ERROR: load_mgh_windows: some frames exceed nframes\n');
    return;
  end
end
...
for frame = frames
  ...
  for slice = slices
    ...
    if(nread ~= nvslice)
      fprintf('ERROR: load_mgh_windows: reading slice %d, frame %d\n',slice,frame);
