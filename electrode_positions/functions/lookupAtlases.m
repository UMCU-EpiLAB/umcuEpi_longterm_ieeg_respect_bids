...
for i=1:size(cfg.hemisphere,2)
    ...
    surface_labels_Destrieux = fullfile(cfg.freesurfer_directory,'label',...
        sprintf('%sh.aparc.a2009s.annot',cfg.hemisphere{i}));
    [~, vert_label_Destrieux.(cfg.hemisphere{i}), colortable_Destrieux] = read_annotation(surface_labels_Destrieux);
    ...
    destrieux_labels = [destrieux_labels, num2cell(numlabels')]; %#ok<AGROW>
    ...
    for kk = 1:size(colortable_Destrieux.table,1) %76 are labels
        ...
        vert_label_Destrieux.(cfg.hemisphere{i})(vert_label_Destrieux.(cfg.hemisphere{i})==colortable_Destrieux.table(kk,5)) = destrieux_labels{corr_numlabel,2}; % unknown
    end
    
    % DKT ATLAS
    % load DKT map
    surface_labels_DKT = fullfile(cfg.freesurfer_directory,'label',...
        sprintf('%sh.aparc.DKTatlas.annot',cfg.hemisphere{i})); % corresponds to DKT40 atlas
    [~, vert_label_DKT.(cfg.hemisphere{i}), colortable_DKT] = read_annotation(surface_labels_DKT);
    ...
    DKT_labels = [DKT_labels, num2cell(numlabels')]; %#ok<AGROW>
    
    % mapping labels to colortable
    for kk = 1:size(colortable_DKT.table,1) %35 are labels
        corr_numlabel = contains(DKT_labels(:,1),colortable_DKT.struct_names{kk})==1;
        
        vert_label_DKT.(cfg.hemisphere{i})(vert_label_DKT.(cfg.hemisphere{i})==colortable_DKT.table(kk,5)) = DKT_labels{corr_numlabel,2};
    end
    ...
    surface_labels_name = fullfile(cfg.freesurfer_directory,'surf',...
        sprintf('%sh.wang15_mplbl.mgz',cfg.hemisphere{i}));
    if exist(surface_labels_name,'file')
        surface_labels = MRIread(surface_labels_name);
        vert_label_Wang.(cfg.hemisphere{i}) = surface_labels.vol(:);
    end
    ...
    surface_labels_name = fullfile(cfg.freesurfer_directory,'surf',...
        sprintf('%sh.benson14_varea.mgz',cfg.hemisphere{i}));
    if exist(surface_labels_name,'file')
        surface_labels_B = MRIread(surface_labels_name);
        vert_label_Benson.(cfg.hemisphere{i}) = surface_labels_B.vol(:);
    end
    clear surface_labels_name
    
    % load Benson Eccen
    surface_labels_name = fullfile(cfg.freesurfer_directory,'surf',...
        sprintf('%sh.benson14_eccen.mgz',cfg.hemisphere{i}));
    if exist(surface_labels_name,'file')
        surface_labels_B = MRIread(surface_labels_name);
        vert_eccen_label.(cfg.hemisphere{i}) = surface_labels_B.vol(:);
        clear surface_labels surface_labels_name
    end
    
    %load Benson Angle
    surface_labels_name = fullfile(cfg.freesurfer_directory,'surf',...
        sprintf('%sh.benson14_angle.mgz',cfg.hemisphere{i}));
    if exist(surface_labels_name,'file')
        surface_labels_B = MRIread(surface_labels_name);
        vert_angle_label_B.(cfg.hemisphere{i}) = surface_labels_B.vol(:);
        clear surface_labels surface_labels_name
    end
    
    % load Benson Sigma
    surface_labels_name = fullfile(cfg.freesurfer_directory,'surf',...
        sprintf('%sh.benson14_sigma.mgz',cfg.hemisphere{i}));
    if exist(surface_labels_name,'file')
        surface_labels_B = MRIread(surface_labels_name);
        vert_sigma_label.(cfg.hemisphere{i}) = surface_labels_B.vol(:);
        clear surface_labels surface_labels_name
    end
...
for elec = 1:size(elecmatrix,1) % loop across electrodes
    ...
    if ~isnan(B(1))
        ...
        localized_electrodes = vert_label_Destrieux.(hemisphere)(I(1));
    else
...
%     localized_electrodes = mode(vert_label_DKT(b<electrode_to_vertex_dist));
    if ~isnan(B(1))
        %take the label of the area with the shortest distance
        localized_electrodes = vert_label_DKT.(hemisphere)(I(1));
    else
        localized_electrodes = NaN;
    end
    ...
    if Benson_label(elec,1)>0
        ...
        eccen_of_electrode = mean(vert_eccen_label.(hemisphere)(b<electrode_to_vertex_dist));
        ...
        angle_of_electrode = mean(vert_angle_label_B.(hemisphere)(b<electrode_to_vertex_dist));
        ...
        sigma_of_electrode = mean(vert_sigma_label.(hemisphere)(b<electrode_to_vertex_dist));
