...
for ii=1:size(cfg(1).ses_dir,2)

    sub_temp = extractBetween(cfg(1).ses_dir{ii},'/sub','/ses');
    sub_label = ['sub', sub_temp{1}];
    ses_label = ['ses', extractAfter(cfg(1).ses_dir{ii},'/ses')];

    coord_json_name = fullfile(cfg(1).ses_dir{ii},'ieeg',...
        [sub_label '_' ses_label '_coordsystem.json']);

    % This line is to create a variable for the name of the file intendedFor
    filename_T1w = fullfile(sub_label,ses_label, 'anat', [sub_label '_' ses_label '_rec-deface_T1w.nii']);

    if exist(fullfile(cfg(ii).proj_diroutput,filename_T1w),'file')
